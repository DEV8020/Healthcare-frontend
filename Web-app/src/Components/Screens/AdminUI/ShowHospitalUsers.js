...
const ShowHospitalUsers = (props) => {
    const [showUpdateCredentialPopup, setShowUpdateCredentialPopup] = useState(false);
const [UpdateCredentialUserId, setUpdateCredentialUserId] = useState("");
  ...
  const handleCredentialPopupUpdate = (UpdateCredentials) => {
    ...
    props.setAlertMessage(UpdateCredentialUserId + " Credentials Updated successfully");
    setShowUpdateCredentialPopup(false);
    props.setAlertFlag(true);

    props.setAlertMessage(" Updated Credentials are : " + UpdateCredentials.updatedUserId +"  , "+ UpdateCredentials.updatedUserPassword);
  };
  ...
  return (
    <div className={classes.center}>
      ...
      {showUpdateCredentialPopup && <UpdateCredentialPopup onUpdate={handleCredentialPopupUpdate} onClose={handleCredentialPopupClose} />}
    </div>
  );
};
